...
function getTime(date) {
  ...
  const time = strDate.match(/\d\d:\d\d:\d\d/);
  return time;
}
...
function getDayName(date) {
  const nameWeekDay = new Date(date).getDay();
  let result = '';
  switch (nameWeekDay) {
    case 0:
      result = 'Sunday';
...
function getNextFriday(date) {
  ...
  const newWeek = new Date(copyDate);
  if (newWeek.getDay() === 5) {
    newWeek.setDate(newWeek.getDate() + 7);
    return newWeek;
  }
  while (newWeek.getDay() !== 5) {
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const itog = Math.floor(diff / (1000 * 60 * 60 * 24));
  return itog + 1;
}
